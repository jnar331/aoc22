...
long long subtract(long long first, long long second){
    return first - second;
}
...
bool testDivide(long long first, long long second){
    return (first % second == 0);
}

struct Monkey{
    bool squareItem;
    long long (*op)(long long f,long long s);
    bool (*test)(long long f,long long s);
    int opArg;
    int testArg;
    long long items[128];
    size_t itemCount;
    ...
    for(int i = 0; i<10000; i++){
        for(int j = 0; j<monkeyCount; j++){
            ...
            while(monkey->itemCount > 0){
                long long secondArg = monkey->opArg;
                if(monkey->squareItem){
                    secondArg = monkey->items[0];
                }

                long long newWorryLvl = monkey->op(monkey->items[0],secondArg) % modProduct;
