...
void getSmallestDir(struct Directory* rootDir, int threshold, int* smallestDir){
    
    int dirSize = getDirSize(rootDir);
    printf("DirSize: %d\n", dirSize);
    if(dirSize >= threshold){
        if(dirSize < *smallestDir){
            *smallestDir = dirSize;
        }
    }

    for(int i = 0; i<rootDir->dirCount; i++){
        getSmallestDir(&rootDir->childs[i],threshold,smallestDir);
    }
}
int main(){
    ...
    const int minUpdateSize = 30000000;
    int smallestDir = totalDiskSpace;
    int spaceToDelete = minUpdateSize - (totalDiskSpace - getDirSize(curDir));
    

    getSmallestDir(curDir,spaceToDelete,&smallestDir);
